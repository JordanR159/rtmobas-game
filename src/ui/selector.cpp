...
Selector::Selector() {
    ...
    this->control_groups_tile_entities = (std::vector<TileEntity *> *)rpmalloc(NUMBER_CONTROL_GROUPS * sizeof(std::vector<TileEntity *>));
    this->control_groups_entities = (std::vector<Entity *> *)rpmalloc(NUMBER_CONTROL_GROUPS * sizeof(std::vector<Entity *>));
}
